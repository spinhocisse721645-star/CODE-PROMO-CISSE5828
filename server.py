...
def calculate_match_minute_fallback(match):
    ...
    avec simulation d'une pause de mi-temps de 15 minutes.
    """
    ...
    try:
        ...
        if elapsed_minutes <= HALF_DURATION:
            return max(1, elapsed_minutes)

        # Pause de mi-temps simulée : le compteur reste bloqué à 45
        if elapsed_minutes <= HALF_DURATION + BREAK_DURATION:
            return HALF_DURATION

        # Deuxième mi-temps : on retire la pause du calcul
        second_half_elapsed = elapsed_minutes - HALF_DURATION - BREAK_DURATION

        return min(90, HALF_DURATION + second_half_elapsed)
