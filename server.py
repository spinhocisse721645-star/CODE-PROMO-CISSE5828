...
# réelle dans ses réponses, et ne signale pas non plus de
# façon fiable le passage en pause de mi-temps (le statut
# reste souvent "IN_PLAY" pendant la pause).
#
# On simule donc une pause fixe de 15 minutes entre la 45e
# minute et la reprise de la seconde période, pour éviter
# que le compteur ne continue de grimper pendant la mi-temps.
# ============================================================
...
def calculate_match_minute_fallback(match):
    ...
    Ajustements pour coller davantage à la réalité :
    - La 1ère mi-temps est souvent prolongée de quelques minutes
      (temps additionnel) avant que l'arbitre siffle la pause.
      On laisse donc le compteur continuer un peu au-delà de 45
      avant de simuler la pause.
    - La 2e mi-temps peut également dépasser 90 minutes.
    """
    ...
    try:
        ...
        HALF_DURATION = 45
        FIRST_HALF_BUFFER = 4      # temps additionnel moyen 1ère mi-temps
        BREAK_DURATION = 15        # pause de mi-temps
        SECOND_HALF_BUFFER = 6     # temps additionnel moyen 2e mi-temps

        first_half_end = HALF_DURATION + FIRST_HALF_BUFFER
        break_end = first_half_end + BREAK_DURATION

        # Statut explicitement en pause (rare, mais on le respecte si présent)
        if status == "PAUSED":
            return min(elapsed_minutes, HALF_DURATION)

        # Première mi-temps (+ temps additionnel)
        if elapsed_minutes <= first_half_end:
            return max(1, min(elapsed_minutes, HALF_DURATION))

        # Pause de mi-temps simulée
        if elapsed_minutes <= break_end:
            return HALF_DURATION

        # Deuxième mi-temps
        second_half_elapsed = elapsed_minutes - break_end

        max_minute = HALF_DURATION + HALF_DURATION + SECOND_HALF_BUFFER

        return min(max_minute, HALF_DURATION + second_half_elapsed)
